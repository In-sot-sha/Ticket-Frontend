...
const Login = () => {
  ...
  // Show expired token message if redirected from token check
  useEffect(() => {
  ...
  // NOTE: Auth verification now happens at App.tsx top-level (AuthProvider)
  // - If user is already authenticated, PublicRoute will redirect to home
  // - If token expired but can be refreshed, it happens silently on app startup
  // - No need for page-level useEffect checking Neon session here
        ...
        >
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden shadow-sm">
              ...
              {/* Password */}
              <div className="relative">
                <label htmlFor="password" className="absolute top-2.5 left-4 text-[10px] font-bold text-neutral-450 dark:text-neutral-500 uppercase tracking-wide">Password</label>
                <input id="password" name="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 pt-6 pb-2 text-sm bg-transparent border-0 focus:ring-0 focus:outline-none text-neutral-800 dark:text-neutral-100 pr-12" placeholder="••••••••" />
                <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-250 transition-colors" onClick={() => setShowPassword(!showPassword)}>
            ...
            <div className="pt-2">
              <button type="submit" disabled={loading || googleLoading} className="w-full h-12 bg-gradient-to-r from-rose-500 via-rose-600 to-pink-600 text-white rounded-xl text-sm font-bold shadow-md hover:shadow-lg transition-transform active:scale-[0.98] duration-150 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
